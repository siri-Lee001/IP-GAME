...
- 可选视频 API 能力模板：`providers/video.provider.json`。该文件只能描述 provider 能力层，不得包含真实 endpoint、模型私有参数、账号报文、密钥名、token、签名 URL 或用户专属请求体。
...
6. 视频：让用户选择视频 API、本地已有视频、或本地静态合成；若选 API，必须先按 provider 能力分级选择多参考图、首尾帧、单参考图或纯文本策略。静态合成只能视为测试/保底，不能冒充高质量成片。
...
- `story.nodes[].media.storyboardImage`：视频参考合成故事板，左侧角色/道具身份参考区，右侧 3x3 连续关键帧。
...
- 分镜图：一张图内包含左侧角色/道具身份参考区 + 右侧 3x3 连续关键帧，兼容单参考图视频 API；画面无文字、编号、水印。
- 多角色分镜图必须启用“角色差异矩阵”：每个主要角色都要锁定脸型轮廓、五官比例、发际线、发型剪影、年龄感、身高体态、姿态语言、服装主色/材质和唯一道具；同性别角色同场时差异要加倍，禁止兄弟脸、双胞胎脸、同款发型和同款白衣换装。
...
1. 视频 API 生成：可参考 `providers/video.provider.json` 的能力分级。必须由用户明确确认；真实 provider 配置只放项目本地未追踪文件或运行时环境，密钥、endpoint、账号报文和用户专属 payload 不进仓库。
...
- **多参考图优先**：能传多张 `reference_image` 时，分别传角色定妆图与节点分镜图，并在 prompt 中绑定 `图1=角色A，图2=角色B，图N=本节点分镜`。这是角色一致性最佳方案。
- **首尾帧 API**：生成并传入同一角色锚点的首帧和尾帧；复杂节点拆成多个短片再拼接。
- **单参考图 API**：先生成“视频参考合成图”，把角色身份条和本节点关键帧合在一张图；每段只聚焦少数角色，避免多人近景。
- **纯文生视频 API**：不能保证精确角色一致性，默认只作为 `prototype` 或风格预览；用强身份锚点、单角色镜头和节点海报补足连续性。
- API 额度紧张时，新任务默认 `duration <= 8s`；禁止重复提交已成功或正在运行的任务；发现同脸/换脸时先诊断原因，不盲目重刷。
- API 报额度不足、权限不足或能力不匹配时，立即停止新增任务并说明降级方案；不要为了测试反复消耗额度。
...
- 首页信息层默认展开，故事简介默认展开；信息层必须半透明、可整体收起，收起后只保留轻量展开按钮，避免长期遮挡海报主体。
- 首次进入引导弹窗。
- 节点视频播放，结束后显示选择。
- 结局页和结局图。
